...
<div class="col-lg-11 col-md-11" style="background: linear-gradient(#a6cc55 0, #e6d45e 100%) !important; padding-left: 150px;">
   <!-- Categories Section Begin -->
    <section class="categories categories-grid spad" style="padding-top: 0; padding-bottom: 50px;">
        <div class="categories__post" style="padding-left: 20px;">
                        		...
                        		<div class="row">
                                    ...
                                    <div class="col-lg-9 col-md-9 col-sm-9">
                                        	...
                                        	<c:choose>
												<c:when test="${q.q_Answer == false}">
											  		<li style="margin-left: 20px; margin-right: 10px;"><button data-parent="#accordion" data-toggle="collapse" data-target="#collapseExample${i-1}" aria-expanded="false" 
                                                		aria-controls="collapseExample" style="background: #ffffff; border-color: #a79e9e;">내용 더보기</button></li>
												</c:when>
												<c:when test="${q.q_Answer == true}">
                                                	<li style="margin-left: 20px; margin-right: 10px;"><button data-parent="#accordion" data-toggle="collapse" data-target="#collapseExample${i-1}" aria-expanded="false" 
                                                		aria-controls="collapseExample" style="background: #ffffff; border-color: #a79e9e;">내용 더보기 + 답글</button></li>
                                            	</c:when>
                                         	</c:choose>
                                        ...
                                        <div class="collapse" id="collapseExample${i-1}">
                                       		...
                                       		<div style="font-size: 15px; padding-bottom: 40px; padding-left: 12px; padding-top: 30px;">
                                       			${q.q_Content}
                                       		</div>
    ...
    <div style="display: block; text-align: center; padding-bottom: 10px; width: 1130px; padding-left: 0px;">		
		<c:choose>
			...
			<c:when test="${paging.startPage != 1 && empty paging.q_Content}">	
				<a class="paging" href="/adminQnaList?nowPage=${paging.startPage - 1 }&cntPerPage=${paging.cntPerPage}">[이전]</a>
			</c:when>
		</c:choose>
		<c:forEach begin="${paging.startPage }" end="${paging.endPage }" var="p">
			<c:choose>
				...
				<c:when test="${p != paging.nowPage && paging.q_Content != null}">
					<a class="paging" href="/adminQnasearch?nowPage=${p }&cntPerPage=${paging.cntPerPage}&searchBox=${paging.q_Ptitle}&searchText=${paging.q_Content}&searchAnswer=${paging.q_Answer}">${p }</a>
				</c:when>
				<c:when test="${p != paging.nowPage && empty paging.q_Content}">
					<a class="paging" href="/adminQnaList?nowPage=${p }&cntPerPage=${paging.cntPerPage}">${p }</a>
				</c:when>
			</c:choose>
		</c:forEach>
		<c:choose>
