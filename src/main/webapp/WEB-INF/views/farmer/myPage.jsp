					...
					<div class="side-bar">
						<div class="user-info">
							...
							<ul class="meta list list-unstyled">
								<li class="name">${member.f_Name}</li>
								<li class="email">${member.f_Id}</li>
							</ul>
						</div>
					</div>
									...
									<tbody>
										<tr>
											...
											<td class="table__td"><fmt:formatDate
													value="${farmer.f_Date}" pattern="yyyy/MM/dd" />
										</tr>
									</tbody>
								</table>
		...
		<div class="modal-dialog modal-confirm">
			<div class="modal-content">
				...
				<div class="modal-body">
					<p>결제 내역이 있는 회원님의 경우 ~~을 위해 &nbsp;&nbsp;&nbsp;수집된 개인정보를 6개월간
						보관 후 파기합니다.</p>
				</div>
				<div class="modal-footer justify-content-center">
					...
					<c:choose>
						<c:when test="${empty mypay}">
							<button
								onclick="location.href='/farmerDelete?f_Id=${member.f_Id}'"
								type="submit" class="btn btn-danger" formaction="/farmerDelete">탈
								퇴</button>
						</c:when>
						<c:when test="${!empty mypay}">
							<button
								onclick="location.href='/farmerDelete?f_Id=${member.f_Id}'" id="fDel"
								type="submit" class="btn btn-danger" formaction="/farmerDelete">탈
								퇴</button>
						</c:when>
					</c:choose>
